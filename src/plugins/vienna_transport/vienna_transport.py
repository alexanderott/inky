...
class ViennaTransport(BasePlugin):
    ...
    def _fetch_departure_data(self, stops_config):
        ...
        try:
            ...
            logger.info("Falling back to individual RBL requests")
            return self._fetch_departure_data_fallback(stops_config)
    ...
    def _fetch_departure_data_fallback(self, stops_config):
        """Fallback method using individual RBL requests (original implementation)."""
        departure_data = []
        ...
        for stop_id, stop_info in stops_config.items():
            try:
                ...
                # Initialize combined stop data
                combined_stop_data = {
                    'name': None,
                    'lines': {}
                }

                # Fetch data for each RBL number individually
                for rbl_number in rbl_numbers:
                    rbl_number = rbl_number.strip()
